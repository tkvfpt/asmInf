...
List<Products> listP;
	try{
	hb.beginTransaction();
	 listP=hb.getEntity("Products",0,12);
	                ...
	                <div class="col-md-3 col-sm-6">
	                    <div class="single-shop-product">
	                        ...
	                        <div class="product-option-shop">
	                            <a class="add_to_cart_button" data-quantity="1" data-product_sku="" data-product_id="70" rel="nofollow" href="productListController?action=AddToCart&product-code=${item.code}">Add to cart</a>
	                        </div>                       
	                    </div>
	                </div>
                </c:forEach>
                    ...
                    <div class="product-pagination text-center">
                        <nav>
                          <ul class="pagination">
                            ...
                            List<Products> listP1;
                            	try{
                            	hb1.beginTransaction();
                            	 listP1=hb1.getEntity("Products");
                            	hb1.commitTransaction();
                            	}finally{
                            		hb1.closeTransaction();
                            	}
                            	int numbOfPages = 1;
                            for(int i=1 ; i<=listP1.size();i++){
                            	int level = i*12;
                            	if(listP1.size()!=level){
                            		//if the number of products is small than the level of 12*i then we break
	                            	if(listP1.size()<level){
	                            		break;
	                            	}else if(listP1.size()>level){
	                            		//if the number of products is greater than the level of 12*i then we must check which level it's at so we can keep +1 into the number of pages and then we break with the previous if 
	                            		numbOfPages+=1;
	                            		continue;
	                            	}
                            	}
                            	else if(listP1.size()==level){
                            		numbOfPages = level/12;
                            		break;
                            	}
                            }
                            %>
                            <%for(int j=1;j<=numbOfPages;j++){%>
                            <c:set var="count" value="<%=j %>"/>
                            	<li><a href="productListController?action=page&count=${count}"><%=numbOfPages %></a></li>	
                            <% } %>
